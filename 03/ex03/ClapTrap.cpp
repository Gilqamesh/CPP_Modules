...
ClapTrap::ClapTrap(): name("Anonymous"), attack_damage(0), hit_points(10), energy_points(10)
{
	std::cout << "ClapTrap default constructor is called from " << name << std::endl;
}

ClapTrap::ClapTrap(std::string n): name(n), attack_damage(0), hit_points(10), energy_points(10)
{
	std::cout << "ClapTrap constructor with string parameter is called from "
		<< name << std::endl;
}

ClapTrap::ClapTrap(std::string n, int hp, int ep, int ad):
	name(n), attack_damage(ad), hit_points(hp), energy_points(ep)
{
	std::cout << "ClapTrap constructor with 4 params is called from " << name << std::endl;
}
